...
func (op OpCode) IsDup() bool {
	switch op {
	case DUP1, DUP2, DUP3, DUP4, DUP5, DUP6, DUP7, DUP8, DUP9, DUP10, DUP11,
...
func (op OpCode) IsControlFlow() bool {
	switch op {
	case JUMP, JUMPI, RETURN, SELFDESTRUCT, STOP:
		return true
	}
	return false
}
...
func (op OpCode) OperandSuffix() int {
	...
	if op.IsDup() {
		return int(byte(op) - byte(DUP1) + 1)
	}
	if op.IsSwap() {
		return int(byte(op) - byte(SWAP1) + 1)
	}
	if op.IsLog() {
		return int(byte(op) - byte(LOG0))
	}
	return 0
}
